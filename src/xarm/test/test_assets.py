...
REQUIRED_FILES = [
    ...
    "xarm_ros2_arm_only/moveit_sequence.py",
...
def test_controllers_yaml_targets_repository_joints():
    ...
    assert gripper == ["gripper_1_joint", "gripper_2_joint"]
...
def test_initial_positions_cover_all_arm_joints():
    ...
    assert {"gripper_1_joint", "gripper_2_joint"}.issubset(positions)
...
def test_capture_world_defines_a_gazebo_video_camera():
    root = ElementTree.parse(PACKAGE_ROOT / "worlds/arm_only_capture.sdf").getroot()
    world = root.find("world")
    assert world is not None
    sensor = world.find("model/link/sensor[@type='camera']")
    assert sensor is not None
    assert sensor.find("plugin[@filename='gz-sim-camera-video-recorder-system']") is not None
...
def test_xacro_anchors_the_arm_to_world_when_requested():
    text = (PACKAGE_ROOT / "urdf/arm_only_xarm.urdf.xacro").read_text()
    assert 'xacro:if value="$(arg include_world_joint)"' in text
    assert '<joint name="world_joint" type="fixed">' in text
    assert '<child link="base_link" />' in text
...
def test_moveit_sequence_uses_the_documented_named_state_order():
    text = (PACKAGE_ROOT / "xarm_ros2_arm_only/moveit_sequence.py").read_text()
    sequence = re.findall(r'Step\(\s*"([^"]+)"', text)
    assert sequence[:5] == ["Home", "Down", "Open_gripper", "Close_gripper", "Home"]
    assert "MoveIt sequence completed" in text
    assert "motion.start_state.is_diff = False" in text
    assert "max(0.0, min(0.68" in text
    assert 'GRIPPER_JOINTS = ("gripper_1_joint", "gripper_2_joint")' in text
    assert "JointTrajectoryPoint" in text
