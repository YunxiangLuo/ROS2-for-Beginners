...
**闭环工作流程**：
1. 传感器采集原始数据 → 感知模块输出障碍物列表、车道线、交通标志
2. 行为决策根据感知结果和路由信息选择驾驶行为
3. 运动规划生成平滑无碰撞的轨迹
4. 控制模块将轨迹转换为油门、制动、转向指令
5. 车辆执行后通过里程计/IMU反馈实际状态，形成闭环
...
**关键指标**：
- **TTC（Time to Collision）**：碰撞时间阈值，安全时 ≥ 3.0s，警告时 < 2.0s，危险时 < 1.0s
- **最小安全距离**：`d_min = v_rel * t_reaction + d_stop`
...
**接管过渡阶段**：
1. **预警阶段（0-1s）**：声光警报，提示驾驶员准备接管
2. **过渡阶段（1-3s）**：逐渐降低车速，保持车道
3. **移交完成**：控制权移交，系统进入监控模式
...
**Jerk等级划分**：
- 🟢 优秀: RMS Jerk < 1.0 m/s³（舒适如豪华轿车）
- 🟡 良好: RMS Jerk 1.0-2.0 m/s³（可接受）
- 🟠 一般: RMS Jerk 2.0-4.0 m/s³（乘客感到不适）
- 🔴 较差: RMS Jerk > 4.0 m/s³（急加速/急刹车）
...
1. **感知边界**：远距离小目标（< 10像素）、遮挡目标、反射/眩光
2. **决策边界**：多目标冲突（同时避让行人和车辆）、无明确路权场景
3. **控制边界**：高速急弯、低附着力路面（湿滑/冰雪）
4. **系统边界**：高CPU负载时实时性、内存不足处理、节点崩溃恢复
5. **时序边界**：消息时序错乱、时间戳跳跃、异步到达
...
**回归触发条件**：
- 代码提交（pre-commit hook 运行快速测试子集）
- PR创建（运行完整测试集，约30分钟）
-  nightly build（运行全部场景 + 故障注入，约2小时）
...
## 参考课程扩展：场景化测试官方框架与安全验证的国际工程标准（官方教程与全球热门课程精选）

> 本节内容综合翻译自 CARLA ScenarioRunner 官方仓库文档、ASAM OpenSCENARIO 标准说明、Autoware 官方文档（安全设计与验证工具）与 ROS 2 官方文档（日志、诊断与容错），另参考国际课程中功能安全（ISO 26262/SOTIF）的教学性介绍。原文均为英文，此处为中文编译，供课后巩固与进阶阅读。

### 1. 场景化测试的官方框架：ScenarioRunner 与 OpenSCENARIO
