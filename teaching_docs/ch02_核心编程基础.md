...
- `ros2 node list` 中，你自己的节点和仿真节点（`gazebo2_robot_state_publisher` 等）在同一 DDS 域内被自动发现，无需任何中心节点。
- `ros2 node info /listener` 显示其订阅关系；若订阅 `/scan`，用 `ros2 topic list | grep scan` 能看到仿真的话题。
- 用 `--ros-args -r __node:=my_node2` 运行副本，验证命名空间/节点重映射（对应 2.2.2 节）。

### 源码与相关位置

- `src/robot_sim_demo/launch/gazebo2.launch.py`：仿真启动入口
- 节点包模板、`setup.py` 见 2.1-2.2 节示例；真实 `/scan` 话题由 `src/robot_sim_demo/config/gazebo2_bridge.yaml` 桥接发布
