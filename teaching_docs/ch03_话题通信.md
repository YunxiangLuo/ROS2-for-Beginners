...
- `/odom` 的 `position.x/y` 随时间变化（或在 `drive:=false` 时基本为 0），体现发布的持续消息流。
- `ros2 topic info /scan` 显示 `sensor_msgs/msg/LaserScan` 与"Publisher count: 1"，由 `gazebo2_bridge` 桥接（发布端来自 Gazebo 侧）。
- 你的订阅节点回调不断打印扫描帧，与 `ros2 topic hz` 测到的频率一致，验证 QoS 深度和传感器高频话题的订阅关系。

### 源码与相关位置

- 仿真入口：`src/robot_sim_demo/launch/gazebo2.launch.py`
- 桥接话题配置：`src/robot_sim_demo/config/gazebo2_bridge.yaml`（含 `/scan`、`/odom`、`/tf`、`/clock`）
- 相机内参：`src/robot_sim_demo/robot_sim_demo/camera_info_publisher.py`（`/camera/camera_info`）
- 巡航驱动示例（订阅/发布 Twist 逻辑）：`src/robot_sim_demo/robot_sim_demo/patrol_driver.py`
