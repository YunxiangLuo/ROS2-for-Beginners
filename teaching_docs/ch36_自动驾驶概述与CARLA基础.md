...
## 学习目标
- 了解自动驾驶技术发展历程与分级标准
- 掌握自动驾驶系统感知—规划—控制三层架构
- 理解CARLA仿真平台的核心概念与架构
- 学会搭建CARLA仿真环境并了解其基本操作
- 掌握CARLA中的World、Map、Actor、Sensor等核心概念
...
**核心特性**：
- **开源免费**：完整代码托管于GitHub，社区活跃
- **高保真渲染**：基于Unreal Engine 4，提供逼真的光照、反射和物理效果
- **灵活的传感器套件**：RGB相机、深度相机、语义分割相机、LIDAR、雷达、GNSS、IMU等
- **内置交通管理器**：Traffic Manager可控制NPC车辆行为，模拟真实交通流
- **丰富的场景地图**：提供Town01-Town12共12种不同风格的地图
- **Python API**：提供完整的Python接口，支持车辆控制、传感器配置、场景编辑等
- **ROS2集成**：提供carla_ros_bridge实现CARLA与ROS2的无缝对接
...
| 检查项 | 验证命令 | 预期结果 |
|--------|---------|---------|
| GPU驱动 | `nvidia-smi` | 显示GPU信息、驱动版本 |
| OpenGL | `glxinfo -B \| grep "OpenGL renderer"` | 显示GPU型号，非llvmpipe |
| CARLA服务器 | `./CarlaUE4.sh -quality-level=Low` | UE4窗口正常显示 |
| Python API | `python3 -c "import carla; print(carla.__version__)"` | 打印版本号无报错 |
...
| `import carla` 失败 | Python包未正确安装 | `cd ~/carla/PythonAPI/carla && pip install -e .` |
...
- 自动驾驶分级标准（SAE J3016 L0-L5）定义了从纯人工到全自动的六个等级
- 典型自动驾驶系统遵循**感知→规划→控制**三层架构
- CARLA是基于UE4的开源自动驾驶仿真器，提供高保真的仿真环境
- CARLA的核心概念包括World、Map、Actor、Blueprint、Sensor和Traffic Manager
- CARLA提供12种不同风格的地图（Town01-Town12）和丰富的动态天气系统
- 支持通过Python API进行完整的仿真控制，包括车辆生成、传感器配置和天气控制
