...
本章学习目标包括：理解行为决策在自动驾驶分层架构中的位置与作用，掌握行为规划与运动规划的分工；掌握用有限状态机（FSM）实现行为决策的方法，包括状态定义、状态转移表与转移逻辑的实现要点；学会使用 CARLA 交通灯 API 识别灯色并判断停止线距离，理解多路口受控灯的筛选逻辑；了解 CARLA 内置 Traffic Manager 的架构与核心参数，能够配置不同驾驶风格的背景交通流；熟悉 Scenario Runner 场景测试框架，能运行内置场景并编写行人横穿、前车切入等自定义场景。
...
其中，**行为规划**（Behavioral Planning）负责离散的驾驶决策（如"跟车"、"变道"），时间尺度为 100ms~1s；**运动规划**（Motion Planning）负责连续的轨迹生成，时间尺度为 10ms~100ms。两者逐级细化、衔接配合，共同构成从决策到执行的完整链路。
...
### 43.4.5 官方要点——Traffic Manager 官方架构：阶段流水线与混合物理模式
...
### 43.5.6 官方要点——Scenario Runner 与 OpenSCENARIO：场景测试的官方框架
